...
#feature[Exercise 2][Joined gradient descent and Newton method][
  ...
  4. Try to perform a Cholesky decomposition of $B_k$:
    - if the decomposition holds, we obtain $B_k = L L^top$. Then, solve the
    system $L L^top p_k = - g_k$ to obtain the descent direction $p_k$.
  ...
  // *Remark:* To compute the Cholesky factorization and solve the system of equations, you can use the functions *scipy.linalg.cho_factor* and *scipy.linalg.cho_solve*. If the matrix is not positive definite, the function *scipy.linalg.cho_factor* returns the exception *scipy.linalg.LinAlgError*. You can manage this error by defining the direction descent as $-g_k$.
]
