...
== Phase 1 — Data Structures (`State.jl`)
...
== Phase 2 — Variational Kernels (`Kernel.jl`)
...
== Phase 3 — Symmetric Damping (`Damping.jl`)
...
== Phase 4 — Compact Representation (`CompactRepresentation.jl`)
...
== Phase 5 — Optimizer and Line Search (`Optimization.jl`)
...
+ Compares four methods: MS-LBFGS (ours), standard L-BFGS, exact BFGS, and
  L-BFGS-B, all with $L = M = 5$ and a limit of 20,000 iterations.
...
#note[
  *Note:* Final numerical results will be added once code execution over CUTEst
  is complete. The table and figure below are placeholders.
]
...
#figure(
  table(
    ...
    inset: 7pt,
    table.header(
      text(weight: "bold")[Problem],
      text(weight: "bold")[Method],
      text(weight: "bold")[Iter.],
      text(weight: "bold")[$f(x^*)$],
      text(weight: "bold")[Status],
    ),
    [ALLINIT], [MS-LBFGS], [11], [5.744], [Converged],
    [L-BFGS], [15], [5.744], [Converged], [ALLINITU],
    [MS-LBFGS], [11], [5.744], [Converged], [L-BFGS],
    [14], [5.744], [Converged], [AIRCRFTB], [MS-LBFGS],
    [4947], [0.0], [Converged], [L-BFGS], [5200],
    [0.0], [Converged], [3PK], [MS-LBFGS], [10000],
    [---], [Max iter.], [L-BFGS], [10000], [---],
    [Max iter.],
  ),
  caption: [
    Preliminary comparison of MS-LBFGS vs. standard L-BFGS on selected CUTEst
    problems.
  ],
) <tab:comparison>
...
#figure(
  rect(
    ...
    #align(center + horizon)[
      #text(style: "italic")[Placeholder — Dolan-Moré Performance Profile] \
      #v(6pt)
      #text(size: 8.5pt, fill: luma(80))[
        Generated by `GenerateProfile.jl` #sym.arrow `comprehensive_perf_profile.png` \
        To insert: `image("comprehensive_perf_profile.png", width: 80%)`
      ]
    ]
  ],
  caption: [
    Dolan-Moré performance profile comparing MS-LBFGS, standard L-BFGS, exact
    BFGS, and L-BFGS-B. A higher curve indicates greater relative efficiency.
  ],
) <fig:perf>
...
#note[
  *Note:* Conclusions will be completed once the full experimental results over
  CUTEst are available.
]

At the implementation level, preliminary results on quadratic functions and
Rosenbrock confirm:

- The five-phase modular structure allows each component to be validated
  independently before integration into the full optimizer.
- The symmetric damping scheme (@eq:damping) correctly handles negative or
  near-zero curvature cases without discarding the full pair.
- The compact representation (@eq:compact) reduces $H$-vector products to
  $O(L^2 n)$ operations, storing only $L times L$ matrices instead of the
  full $n times n$ Hessian.
- Convergence on the 2D Rosenbrock function and 100-dimensional convex
  quadratics is verified successfully in the unit tests.
...
#v(0.3em)
*Gubarev, F. V.* (2024). A Multi-Secant Limited-Memory BFGS Method.
