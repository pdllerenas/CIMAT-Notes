...
#define SIZEMAX 100005 // Bumped up for 50k + 50k strings
...
void solve()
{
  ...
  while (cin >> k && k != 0)
  {
    ...
    if (getline(cin, line) && line.find("BEGIN TDP CODEBASE") != string::npos)
    {
    ...
    string jcn = "";
    if (getline(cin, line) && line.find("BEGIN JCN CODEBASE") != string::npos)
    {
    ...
    for (const auto &match : candidates)
    {
      ...
      if (!is_contained)
      {
        ...
        cout << jcn.substr(match.jcn_pos, match.len) << "\n";
        count++;
      }
    }
  }
